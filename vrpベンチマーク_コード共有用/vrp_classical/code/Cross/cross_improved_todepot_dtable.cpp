#include "cross_improved_todepot_dtable.hpp"
 ...
 * @return tuple<bool, vector<int>, vector<int>, bool, bool> 改善有無(true : 改善あり)、交換後の配送先の組み合わせ、集荷最適化の有無(true : 最適化あり)
 * @details cross処理後の最終訪問先までの改善有無を判定する関数。
 */
tuple<bool, vector<int>, vector<int>, bool, bool, vector<int>, vector<int>, map<tuple<string, double>, vector<vector<double>>>, map<tuple<string, double>, vector<vector<double>>>, map<tuple<string, string, double>, vector<vector<double>>>, map<tuple<string, double>, vector<vector<double>>>, vector<vector<int>>, vector<Customer>> cross_improved_todepot_dtable(const Vehicle &vehicle1, const Vehicle &vehicle2, vector<Customer> &customers, const int pos1, const int pos2, const int pos3, const int pos4, vector<Vehicle> &vehicles, Cppargs &argvs, double &cdiff, bool no_depo_parkt_flag, bool one_table_flag){
    ...
    if(cust_list1.empty() || cust_list2.empty()){
        return make_tuple(false, custs1_best, custs2_best, false, false, route1_best, route2_best, ttable_best, dtable_best, etable_best, xtable_best, t_route_best, customers_best); //交換対象がデポor休憩スポットのみならfalseを返す
    }
    ...
    //改善前時間違反量を計算
    for(int i = start_idx1; i < veh1_route_size; i++){
        if(i>0){
            if((argvs.bulkShipping_flag == false && (customers.at(vehicle1.route.at(i-1)).custno != customers.at(vehicle1.route.at(i)).custno)) ||
            (argvs.bulkShipping_flag == true && (customers.at(customers.at(vehicle1.route.at(i-1)).tieup_customer.at(0)).custno != customers.at(customers.at(vehicle1.route.at(i)).tieup_customer.at(0)).custno))){
    ...
    btpen += max(0.0, vehicle1.lv.at(veh1_route_size-1)-vehicle1.endhour);

    for(int i = start_idx2; i < veh2_route_size; i++){
        if(i>0){
            if((argvs.bulkShipping_flag == false && (customers.at(vehicle2.route.at(i-1)).custno != customers.at(vehicle2.route.at(i)).custno)) ||
            (argvs.bulkShipping_flag == true && (customers.at(customers.at(vehicle2.route.at(i-1)).tieup_customer.at(0)).custno != customers.at(customers.at(vehicle2.route.at(i)).tieup_customer.at(0)).custno))){
    ...
    btpen += max(0.0, vehicle2.lv.at(veh2_route_size-1)-vehicle2.endhour);


    //改善前の作業終了時刻
    double bv1final_t = vehicle1.lv.at(veh1_route_size-1), bv2final_t = vehicle2.lv.at(veh2_route_size-1);
    ...
    if(one_table_flag == false || argvs.multitrip_flag == true || pick_nondepo_flag == true || argvs.avedist_ae_flag == true || argvs.avedist_ape_flag == true || argvs.break_flag == true){
        bdist1 = calc_dist(vehicle1, customers, start_idx1+1, veh1_route_size, argvs.management_dtable);
        bdist2 = calc_dist(vehicle2, customers, start_idx2+1, veh2_route_size, argvs.management_dtable);
        bdist = bdist1+bdist2;
        
        if(!argvs.management_etable.empty()){
            bpower = calc_power(vehicle1, customers, start_idx1+1, veh1_route_size, argvs.management_etable)
                    +calc_power(vehicle2, customers, start_idx2+1, veh2_route_size, argvs.management_etable);
        }

        if(!argvs.management_xtable.empty()){
            bexp = calc_exp(vehicle1, customers, start_idx1+1, veh1_route_size, argvs.management_xtable)
                +calc_exp(vehicle2, customers, start_idx2+1, veh2_route_size, argvs.management_xtable);
        }
    }
    else{
        bdist1 =  get_dtable(vehicle1.vtype, vehicle1.lv.at(pos1-1), argvs.management_dtable, customers.at(vehicle1.route.at(pos1-1)).spotno, customers.at(vehicle1.route.at(pos1)).spotno)
                +get_dtable(vehicle1.vtype, vehicle1.lv.at(pos2-1), argvs.management_dtable, customers.at(vehicle1.route.at(pos2-1)).spotno, customers.at(vehicle1.route.at(pos2)).spotno);
        bdist2 = get_dtable(vehicle2.vtype, vehicle2.lv.at(pos3-1), argvs.management_dtable, customers.at(vehicle2.route.at(pos3-1)).spotno, customers.at(vehicle2.route.at(pos3)).spotno)
                +get_dtable(vehicle2.vtype, vehicle2.lv.at(pos4-1), argvs.management_dtable, customers.at(vehicle2.route.at(pos4-1)).spotno, customers.at(vehicle2.route.at(pos4)).spotno);
        bdist = bdist1+bdist2;
        if(!argvs.management_etable.empty()){
            bpower = get_etable(vehicle1.vtype, vehicle1.etype, vehicle1.lv.at(pos1-1), argvs.management_etable, customers.at(vehicle1.route.at(pos1-1)).spotno, customers.at(vehicle1.route.at(pos1)).spotno)
                +get_etable(vehicle1.vtype, vehicle1.etype, vehicle1.lv.at(pos2-1), argvs.management_etable, customers.at(vehicle1.route.at(pos2-1)).spotno, customers.at(vehicle1.route.at(pos2)).spotno)
                ...
                +get_etable(vehicle2.vtype, vehicle2.etype, vehicle2.lv.at(pos4-1), argvs.management_etable, customers.at(vehicle2.route.at(pos4-1)).spotno, customers.at(vehicle2.route.at(pos4)).spotno);
        }
        if(!argvs.management_xtable.empty()){
            bexp = get_xtable(vehicle1.vtype, vehicle1.lv.at(pos1-1), argvs.management_xtable, customers.at(vehicle1.route.at(pos1-1)).spotno, customers.at(vehicle1.route.at(pos1)).spotno)
                +get_xtable(vehicle1.vtype, vehicle1.lv.at(pos2-1), argvs.management_xtable, customers.at(vehicle1.route.at(pos2-1)).spotno, customers.at(vehicle1.route.at(pos2)).spotno)
                ...
                +get_xtable(vehicle2.vtype, vehicle2.lv.at(pos4-1), argvs.management_xtable, customers.at(vehicle2.route.at(pos4-1)).spotno, customers.at(vehicle2.route.at(pos4)).spotno);
        }
    }

    if(argvs.opt_t_weight != 0){
        btime = vehicle1.lv.at(veh1_route_size-1)+vehicle2.lv.at(veh2_route_size-1);
    }
	...
	if(argvs.opt_unassigned_flag==true){
		if(vehicle1.reserved_v_flag==true){
            int route_size = vehicle1.route.size();
            for(int rc=0; rc<route_size; rc++){
		...
		if(vehicle2.reserved_v_flag==true){
            int route_size = vehicle2.route.size();
            for(int rc=0; rc<route_size; rc++){
        ...
        for(auto custs2 : insert_list2){
            ...
            if(argvs.break_flag == false){
                ...
                (argvs.bulkShipping_flag == true && pos2 == veh1_route_size-1 && customers.at(customers.at(custs1.at(custs1_size-1)).tieup_customer.at(0)).custno != "0" && customers.at(customers.at(vehicle1.route.at(veh1_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag1 == true)){ //集荷最適化
                    ...
                    adist += get_dtable(vehicle1.vtype, lv_t, argvs.management_dtable, customers.at(custs1.at(custs1_size-1)).spotno, customers.at(0).spotno);
                    if(!argvs.management_etable.empty()){
                        apower += get_etable(vehicle1.vtype, vehicle1.etype, lv_t, argvs.management_etable, customers.at(custs1.at(custs1_size-1)).spotno, customers.at(0).spotno);
                    }
                    if(!argvs.management_xtable.empty()){
                        aexp += get_xtable(vehicle1.vtype, lv_t, argvs.management_xtable, customers.at(custs1.at(custs1_size-1)).spotno, customers.at(0).spotno);
                    }

                    arr_t = lv_t+get_ttable(vehicle1.vtype, lv_t, argvs.management_ttable, customers.at(custs1.at(custs1_size-1)).spotno, customers.at(0).spotno)*vehicle1.drskill;
                    st_t = max(arr_t+customers.at(0).parkt_arrive, customers.at(0).ready_fs)+customers.at(0).parkt_open;
                    atpen += max(0.0, st_t-customers.at(0).due_fs);
                    lv_t = max(arr_t+customers.at(0).parkt_arrive, customers.at(0).ready_fs)+(servt_dict1.at(veh1_route_size-1)+vehicle1.depotservt)*vehicle1.opskill;
                    lv_t += customers.at(0).parkt_open;

                    if(get_dtable(vehicle1.vtype, lv_t, argvs.management_dtable, customers.at(0).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno) == -1
                    || get_ttable(vehicle1.vtype, lv_t, argvs.management_ttable, customers.at(0).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno) == -1
                    || (!argvs.management_etable.empty() && get_etable(vehicle1.vtype, vehicle1.etype, lv_t, argvs.management_etable, customers.at(0).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno) == -1)
                    || (!argvs.management_xtable.empty() && get_xtable(vehicle1.vtype, lv_t, argvs.management_xtable, customers.at(0).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno) == -1)){
                        continue; //通行不可
                    }

                    adist += get_dtable(vehicle1.vtype, lv_t, argvs.management_dtable, customers.at(0).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno);
                    if(!argvs.management_etable.empty()){
                        apower += get_etable(vehicle1.vtype, vehicle1.etype, lv_t, argvs.management_etable, customers.at(0).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno);
                    }
                    if(!argvs.management_xtable.empty()){
                        aexp += get_xtable(vehicle1.vtype, lv_t, argvs.management_xtable, customers.at(0).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno);
                    }

                    arr_t = lv_t+get_ttable(vehicle1.vtype, lv_t, argvs.management_ttable, customers.at(0).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno)*vehicle1.drskill;
                    st_t = max(arr_t, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs);
                    atpen += max(0.0, st_t-customers.at(vehicle1.route.at(veh1_route_size-1)).due_fs);
                    lv_t = max(arr_t, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs);
                    arr = arr_t;
                    lv = lv_t;
                    st = st_t;
                }
                ...
                (argvs.bulkShipping_flag == true && pos2 == veh1_route_size-2 && customers.at(customers.at(vehicle1.route.at(veh1_route_size-2)).tieup_customer.at(0)).custno == "0" && customers.at(customers.at(vehicle1.route.at(veh1_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag1 == false)){ //集荷最適化
                    ...
                    adist += get_dtable(vehicle1.vtype, lv_t, argvs.management_dtable, customers.at(custs1.at(custs1_size-1)).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno);
                    if(!argvs.management_etable.empty()){
                        apower += get_etable(vehicle1.vtype, vehicle1.etype, lv_t, argvs.management_etable, customers.at(custs1.at(custs1_size-1)).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno);
                    }
                    if(!argvs.management_xtable.empty()){
                        aexp += get_xtable(vehicle1.vtype, lv_t, argvs.management_xtable, customers.at(custs1.at(custs1_size-1)).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno);
                    }

                    arr_t = lv_t+get_ttable(vehicle1.vtype, lv_t, argvs.management_ttable, customers.at(custs1.at(custs1_size-1)).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno)*vehicle1.drskill;
                    st_t = max(arr_t, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs);
                    atpen += max(0.0, st_t-customers.at(vehicle1.route.at(veh1_route_size-1)).due_fs);
                    lv_t = max(arr_t, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs);
                    arr = arr_t;
                    ...
                    if(one_table_flag == false || argvs.multitrip_flag == true || pick_nondepo_flag == true || argvs.avedist_ae_flag == true || argvs.avedist_ape_flag == true){
                        passable = check_impassability(vehicle1, customers, lv, pos2+1, veh1_route_size-1, servt_dict1, adist, apower, aexp, no_park_idx1, pick_opt_flag1, argvs);
                    }
                ...
                if(pos2 == veh1_route_size-1){ //pos2が最終訪問先の場合
                    if((argvs.bulkShipping_flag == false && (customers.at(custs1.at(custs1_size-1)).custno != customers.at(vehicle1.route.at(veh1_route_size-1)).custno)) ||
                    (argvs.bulkShipping_flag == true && (customers.at(customers.at(custs1.at(custs1_size-1)).tieup_customer.at(0)).custno != customers.at(customers.at(vehicle1.route.at(veh1_route_size-1)).tieup_customer.at(0)).custno))){
                        atpen += max(0.0, st1-customers.at(vehicle1.route.at(veh1_route_size-1)).due_fs);
                    }
                    ...
                    if(!((argvs.bulkShipping_flag == false && pos2 == veh1_route_size-2 && customers.at(vehicle1.route.at(veh1_route_size-2)).custno == "0" && customers.at(vehicle1.route.at(veh1_route_size-1)).custno != "0" && pick_opt_flag1 == false) ||
                    (argvs.bulkShipping_flag == true && pos2 == veh1_route_size-2 && customers.at(customers.at(vehicle1.route.at(veh1_route_size-2)).tieup_customer.at(0)).custno == "0" && customers.at(customers.at(vehicle1.route.at(veh1_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag1 == false)) &&
                    !((argvs.bulkShipping_flag == false && pos2 == veh1_route_size-1 && customers.at(custs1.at(custs1_size-1)).custno != "0" && customers.at(vehicle1.route.at(veh1_route_size-1)).custno != "0" && pick_opt_flag1 == true) ||
                    (argvs.bulkShipping_flag == true && pos2 == veh1_route_size-1 && customers.at(customers.at(custs1.at(custs1_size-1)).tieup_customer.at(0)).custno != "0" && customers.at(customers.at(vehicle1.route.at(veh1_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag1 == true))){
                        if(customers.at(custs1.at(custs1_size-1)).custno != customers.at(vehicle1.route.at(pos2)).custno){
                            atpen += max(0.0, arr1-customers.at(vehicle1.route.at(pos2)).due_fs);
                        }

                        lv_t = lv1;
                        st_t = st1;

                        for(int i = pos2; i < veh1_route_size-2; i++){
                            if(i+1 == veh1_route_size-2 && customers.at(vehicle1.route.at(veh1_route_size-2)).custno == "0" && customers.at(vehicle1.route.at(veh1_route_size-1)).custno != "0" && pick_opt_flag1 == false){
                                continue;
                            }							
                        ...
                        if((argvs.bulkShipping_flag == false && customers.at(vehicle1.route.at(veh1_route_size-2)).custno != "0" && customers.at(vehicle1.route.at(veh1_route_size-1)).custno != "0" && pick_opt_flag1 == true) ||
                        (argvs.bulkShipping_flag == true && customers.at(customers.at(vehicle1.route.at(veh1_route_size-2)).tieup_customer.at(0)).custno != "0" && customers.at(customers.at(vehicle1.route.at(veh1_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag1 == true)){ //集荷最適化
                            arr_t = lv_t+get_ttable(vehicle1.vtype, lv_t, argvs.management_ttable, customers.at(vehicle1.route.at(veh1_route_size-2)).spotno, customers.at(0).spotno)*vehicle1.drskill;
                            st_t = max(arr_t+customers.at(0).parkt_arrive, customers.at(0).ready_fs)+customers.at(0).parkt_open;
                            atpen += max(0.0, st_t-customers.at(0).due_fs);
                            lv_t = max(arr_t+customers.at(0).parkt_arrive, customers.at(0).ready_fs)+servt_dict1.at(veh1_route_size-1)*vehicle1.opskill;
                        ...
                        else if((argvs.bulkShipping_flag == false && customers.at(vehicle1.route.at(veh1_route_size-2)).custno == "0" && customers.at(vehicle1.route.at(veh1_route_size-1)).custno != "0" && pick_opt_flag1 == false) ||
                        (argvs.bulkShipping_flag == true && customers.at(customers.at(vehicle1.route.at(veh1_route_size-2)).tieup_customer.at(0)).custno == "0" && customers.at(customers.at(vehicle1.route.at(veh1_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag1 == false)){ //集荷最適化
                            arr_t = lv_t+get_ttable(vehicle1.vtype, lv_t, argvs.management_ttable, customers.at(vehicle1.route.at(veh1_route_size-3)).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno)*vehicle1.drskill;
                            lv_t = max(arr_t, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs);
                            st_t = max(arr_t, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs);
                        }
                        else{
                            arr_t = lv_t+get_ttable(vehicle1.vtype, lv_t, argvs.management_ttable, customers.at(vehicle1.route.at(veh1_route_size-2)).spotno, customers.at(vehicle1.route.at(veh1_route_size-1)).spotno)*vehicle1.drskill;
                            if(customers.at(vehicle1.route.at(veh1_route_size-1)).spotid != customers.at(vehicle1.route.at(veh1_route_size-2)).spotid && vector_finder(no_park_idx1, veh1_route_size-1) == false){
                                lv_t = max(arr_t+customers.at(vehicle1.route.at(veh1_route_size-1)).parkt_arrive, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs)+servt_dict1.at(veh1_route_size-1)*vehicle1.opskill;
                                st_t = max(arr_t+customers.at(vehicle1.route.at(veh1_route_size-1)).parkt_arrive, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs)+customers.at(vehicle1.route.at(veh1_route_size-1)).parkt_open;
                            }
                            else{
                                lv_t = max(arr_t, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs)+servt_dict1.at(veh1_route_size-1)*vehicle1.opskill;
                                st_t = max(arr_t, customers.at(vehicle1.route.at(veh1_route_size-1)).ready_fs);
                            }
                            if(customers.at(vehicle1.route.at(veh1_route_size-1)).spotid != customers.at(vehicle1.route.at(veh1_route_size-2)).spotid && vector_finder(no_park_idx1, veh1_route_size-1) == false){
                                lv_t += customers.at(vehicle1.route.at(veh1_route_size-1)).parkt_open; //駐車時間
                            }
                        }
                        ...
                        if((argvs.bulkShipping_flag == false && (customers.at(vehicle1.route.at(veh1_route_size-2)).custno != "0" || customers.at(vehicle1.route.at(veh1_route_size-1)).custno != "0")) ||
                        (argvs.bulkShipping_flag == true && (customers.at(customers.at(vehicle1.route.at(veh1_route_size-2)).tieup_customer.at(0)).custno != "0" || customers.at(customers.at(vehicle1.route.at(veh1_route_size-1)).tieup_customer.at(0)).custno != "0"))){ //デポが連続しない場合
                            atpen += max(0.0, st1-customers.at(vehicle1.route.at(veh1_route_size-1)).due_fs);
                        }
                    }
                ...
                if(customers.at(vehicle2.route.at(pos3-1)).custno != customers.at(custs2.at(0)).custno || customers.at(custs2.at(0)).custno == "0"){
                    ...
                    if(customers.at(custs2.at(0)).spotid != customers.at(vehicle2.route.at(pos3-1)).spotid && (customers.at(custs2.at(0)).custno != "0" || vector_finder(no_park_idx2, distance(vehicle2.route.begin(), find(vehicle2.route.begin()+depo_begin_idx2, vehicle2.route.end(), custs2.at(0)))) == false)){
                        ...
                        st_t = max(arr_t+customers.at(custs2.at(0)).parkt_arrive, customers.at(custs2.at(0)).ready_fs) + customers.at(custs2.at(0)).parkt_open;
                    }
                    else{
                ...
                (argvs.bulkShipping_flag == true && pos4 == veh2_route_size-1 && customers.at(customers.at(custs2.at(custs2_size-1)).tieup_customer.at(0)).custno != "0" && customers.at(customers.at(vehicle2.route.at(veh2_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag2 == true)){ //集荷最適化
                    ...
                    adist += get_dtable(vehicle2.vtype, lv_t, argvs.management_dtable, customers.at(custs2.at(custs2_size-1)).spotno, customers.at(0).spotno);
                    if(!argvs.management_etable.empty()){
                        apower += get_etable(vehicle2.vtype, vehicle2.etype, lv_t, argvs.management_etable, customers.at(custs2.at(custs2_size-1)).spotno, customers.at(0).spotno);
                    }
                    if(!argvs.management_xtable.empty()){
                        aexp += get_xtable(vehicle2.vtype, lv_t, argvs.management_xtable, customers.at(custs2.at(custs2_size-1)).spotno, customers.at(0).spotno);
                    }

                    arr_t = lv_t+get_ttable(vehicle2.vtype, lv_t, argvs.management_ttable, customers.at(custs2.at(custs2_size-1)).spotno, customers.at(0).spotno)*vehicle2.drskill;
                    st_t = max(arr_t+customers.at(0).parkt_arrive, customers.at(0).ready_fs)+customers.at(0).parkt_open;
                    atpen += max(0.0, st_t-customers.at(0).due_fs);
                    lv_t = max(arr_t+customers.at(0).parkt_arrive, customers.at(0).ready_fs)+(servt_dict2.at(veh2_route_size-1)+vehicle2.depotservt)*vehicle2.opskill;
                    lv_t += customers.at(0).parkt_open;

                    if(get_dtable(vehicle2.vtype, lv_t, argvs.management_dtable, customers.at(0).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno) == -1
                    || get_ttable(vehicle2.vtype, lv_t, argvs.management_ttable, customers.at(0).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno) == -1
                    || (!argvs.management_etable.empty() && get_etable(vehicle2.vtype, vehicle2.etype, lv_t, argvs.management_etable, customers.at(0).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno) == -1)
                    || (!argvs.management_xtable.empty() && get_xtable(vehicle2.vtype, lv_t, argvs.management_xtable, customers.at(0).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno) == -1)){
                        continue; //通行不可
                    }

                    adist += get_dtable(vehicle2.vtype, lv_t, argvs.management_dtable, customers.at(0).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno);
                    if(!argvs.management_etable.empty()){
                        apower += get_etable(vehicle2.vtype, vehicle2.etype, lv_t, argvs.management_etable, customers.at(0).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno);
                    }
                    if(!argvs.management_xtable.empty()){
                        aexp += get_xtable(vehicle2.vtype, lv_t, argvs.management_xtable, customers.at(0).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno);
                    }

                    arr_t = lv_t+get_ttable(vehicle2.vtype, lv_t, argvs.management_ttable, customers.at(0).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno)*vehicle2.drskill;
                    st_t = max(arr_t, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs);
                    atpen += max(0.0, st_t-customers.at(vehicle2.route.at(veh2_route_size-1)).due_fs);
                    lv_t = max(arr_t, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs);
                    arr = arr_t;
                    lv = lv_t;
                    st = st_t;
                }
                ...
                (argvs.bulkShipping_flag == true && pos4 == veh2_route_size-2 && customers.at(customers.at(vehicle2.route.at(veh2_route_size-2)).tieup_customer.at(0)).custno == "0" && customers.at(customers.at(vehicle2.route.at(veh2_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag2 == false)){ //集荷最適化
                    ...
                    adist += get_dtable(vehicle2.vtype, lv_t, argvs.management_dtable, customers.at(custs2.at(custs2_size-1)).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno);
                    if(!argvs.management_etable.empty()){
                        apower += get_etable(vehicle2.vtype, vehicle2.etype, lv_t, argvs.management_etable, customers.at(custs2.at(custs2_size-1)).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno);
                    }
                    if(!argvs.management_xtable.empty()){
                        aexp += get_xtable(vehicle2.vtype, lv_t, argvs.management_xtable, customers.at(custs2.at(custs2_size-1)).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno);
                    }

                    arr_t = lv_t+get_ttable(vehicle2.vtype, lv_t, argvs.management_ttable, customers.at(custs2.at(custs2_size-1)).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno)*vehicle2.drskill;
                    st_t = max(arr_t, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs);
                    atpen += max(0.0, st_t-customers.at(vehicle2.route.at(veh2_route_size-1)).due_fs);
                    lv_t = max(arr_t, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs);
                    arr = arr_t;
                    ...
                    if(one_table_flag == false || argvs.multitrip_flag == true || pick_nondepo_flag == true || argvs.avedist_ae_flag == true || argvs.avedist_ape_flag == true){
                        passable = check_impassability(vehicle2, customers, lv, pos4+1, veh2_route_size-1, servt_dict2, adist, apower, aexp, no_park_idx2, pick_opt_flag2, argvs);
                    }
                ...
                if(pos4 == veh2_route_size-1){ //pos4が最終訪問先の場合
                    if((argvs.bulkShipping_flag == false && (customers.at(custs2.at(custs2_size-1)).custno != customers.at(vehicle2.route.at(veh2_route_size-1)).custno)) || 
                    (argvs.bulkShipping_flag == true && (customers.at(customers.at(custs2.at(custs2_size-1)).tieup_customer.at(0)).custno != customers.at(customers.at(vehicle2.route.at(veh2_route_size-1)).tieup_customer.at(0)).custno))){
                        atpen += max(0.0, st2-customers.at(vehicle2.route.at(veh2_route_size-1)).due_fs);
                    }
                    ...
                    if(!((argvs.bulkShipping_flag == false && pos4 == veh2_route_size-2 && customers.at(vehicle2.route.at(veh2_route_size-2)).custno == "0" && customers.at(vehicle2.route.at(veh2_route_size-1)).custno != "0" && pick_opt_flag2 == false) ||
                    (argvs.bulkShipping_flag == true && pos4 == veh2_route_size-2 && customers.at(customers.at(vehicle2.route.at(veh2_route_size-2)).tieup_customer.at(0)).custno == "0" && customers.at(customers.at(vehicle2.route.at(veh2_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag2 == false)) &&
                    !((argvs.bulkShipping_flag == false && pos4 == veh2_route_size-1 && customers.at(custs2.at(custs2_size-1)).custno != "0" && customers.at(vehicle2.route.at(veh2_route_size-1)).custno != "0" && pick_opt_flag2 == true) ||
                    (argvs.bulkShipping_flag == true && pos4 == veh2_route_size-1 && customers.at(customers.at(custs2.at(custs2_size-1)).tieup_customer.at(0)).custno != "0" && customers.at(customers.at(vehicle2.route.at(veh2_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag2 == true))){
                        if(customers.at(custs2.at(custs2_size-1)).custno != customers.at(vehicle2.route.at(pos4)).custno){
                            atpen += max(0.0, st2-customers.at(vehicle2.route.at(pos4)).due_fs);
                        }

                        lv_t = lv2;
                        st_t = st2;

                        for(int i = pos4; i < veh2_route_size-2; i++){
                            if(i+1 == veh2_route_size-2 && customers.at(vehicle2.route.at(veh2_route_size-2)).custno == "0" && customers.at(vehicle2.route.at(veh2_route_size-1)).custno != "0" && pick_opt_flag2 == false){
                                continue;
                            }
                        ...
                        if((argvs.bulkShipping_flag == false && customers.at(vehicle2.route.at(veh2_route_size-2)).custno != "0" && customers.at(vehicle2.route.at(veh2_route_size-1)).custno != "0" && pick_opt_flag2 == true) ||
                        (argvs.bulkShipping_flag == true && customers.at(customers.at(vehicle2.route.at(veh2_route_size-2)).tieup_customer.at(0)).custno != "0" && customers.at(customers.at(vehicle2.route.at(veh2_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag2 == true)){ //集荷最適化
                            arr_t = lv_t+get_ttable(vehicle2.vtype, lv_t, argvs.management_ttable, customers.at(vehicle2.route.at(veh2_route_size-2)).spotno, customers.at(0).spotno)*vehicle2.drskill;
                            st_t = max(arr_t+customers.at(0).parkt_arrive, customers.at(0).ready_fs)+customers.at(0).parkt_open;
                            atpen += max(0.0, st_t-customers.at(0).due_fs);
                            lv_t = max(arr_t+customers.at(0).parkt_arrive, customers.at(0).ready_fs)+servt_dict2.at(veh2_route_size-1)*vehicle2.opskill;
                            lv_t += customers.at(0).parkt_open; //駐車時間
                        ...
                        else if((argvs.bulkShipping_flag == false && customers.at(vehicle2.route.at(veh2_route_size-2)).custno == "0" && customers.at(vehicle2.route.at(veh2_route_size-1)).custno != "0" && pick_opt_flag2 == false) ||
                        (argvs.bulkShipping_flag == true && customers.at(customers.at(vehicle2.route.at(veh2_route_size-2)).tieup_customer.at(0)).custno == "0" && customers.at(customers.at(vehicle2.route.at(veh2_route_size-1)).tieup_customer.at(0)).custno != "0" && pick_opt_flag2 == false)){ //集荷最適化
                            arr_t = lv_t+get_ttable(vehicle2.vtype, lv_t, argvs.management_ttable, customers.at(vehicle2.route.at(veh2_route_size-3)).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno)*vehicle2.drskill;
                            lv_t = max(arr_t, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs);
                            st_t = max(arr_t, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs);
                        }
                        else{
                            arr_t = lv_t+get_ttable(vehicle2.vtype, lv_t, argvs.management_ttable, customers.at(vehicle2.route.at(veh2_route_size-2)).spotno, customers.at(vehicle2.route.at(veh2_route_size-1)).spotno)*vehicle2.drskill;
                            if(customers.at(vehicle2.route.at(veh2_route_size-1)).spotid != customers.at(vehicle2.route.at(veh2_route_size-2)).spotid && vector_finder(no_park_idx2, veh2_route_size-1) == false){
                                lv_t = max(arr_t+customers.at(vehicle2.route.at(veh2_route_size-1)).parkt_arrive, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs)+servt_dict2.at(veh2_route_size-1)*vehicle2.opskill;
                                st_t = max(arr_t+customers.at(vehicle2.route.at(veh2_route_size-1)).parkt_arrive, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs)+customers.at(vehicle2.route.at(veh2_route_size-1)).parkt_open;
                            }
                            else{
                                lv_t = max(arr_t, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs)+servt_dict2.at(veh2_route_size-1)*vehicle2.opskill;
                                st_t = max(arr_t, customers.at(vehicle2.route.at(veh2_route_size-1)).ready_fs);
                            }
                            if(customers.at(vehicle2.route.at(veh2_route_size-1)).spotid != customers.at(vehicle2.route.at(veh2_route_size-2)).spotid && vector_finder(no_park_idx2, veh2_route_size-1) == false){
                                lv_t += customers.at(vehicle2.route.at(veh2_route_size-1)).parkt_open; //駐車時間
                            }
                        }
                        ...
                        if((argvs.bulkShipping_flag == false && (customers.at(vehicle2.route.at(veh2_route_size-2)).custno != "0" || customers.at(vehicle2.route.at(veh2_route_size-1)).custno != "0")) ||
                        (argvs.bulkShipping_flag == true && (customers.at(customers.at(vehicle2.route.at(veh2_route_size-2)).tieup_customer.at(0)).custno != "0" || customers.at(customers.at(vehicle2.route.at(veh2_route_size-1)).tieup_customer.at(0)).custno != "0"))){ //デポが連続しない場合
                            atpen += max(0.0, st2-customers.at(vehicle2.route.at(veh2_route_size-1)).due_fs);
                        }
                    }
                    atpen += max(0.0, lv2-vehicle2.endhour);
                ...
                arr1 = veh1.arr[skv1_route_size-1];
                lv1 = veh1.lv[skv1_route_size-1];
                arr2 = veh2.arr[skv2_route_size-1];
                lv2 = veh2.lv[skv2_route_size-1];
                ...
                int nn1 = veh1.route.size();
                for(int i = start_idx1; i < nn1; i++){
                    if(i>0){
                        if((argvs.bulkShipping_flag == false && (customers.at(veh1.route.at(i-1)).custno != customers.at(veh1.route.at(i)).custno)) ||
                        (argvs.bulkShipping_flag == true && (customers.at(customers.at(veh1.route.at(i-1)).tieup_customer.at(0)).custno != customers.at(customers.at(veh1.route.at(i)).tieup_customer.at(0)).custno))){
				...
				for(int i=start_idx1+1; i<nn1; i++){
                    if((argvs.bulkShipping_flag == false && (customers.at(veh1.route.at(i-1)).custno != customers.at(veh1.route.at(i)).custno)) ||
                        (argvs.bulkShipping_flag == true && (customers.at(customers.at(veh1.route.at(i-1)).tieup_customer.at(0)).custno != customers.at(customers.at(veh1.route.at(i)).tieup_customer.at(0)).custno))){
                            atpen += max(0.0, veh1.arr.at(i) - customers.at(veh1.route.at(i)).due_fs);
                    }
                }
                atpen += max(0.0, lv1-veh1.endhour);
                ...
                int nn2 = veh2.route.size();
                for(int i = start_idx2; i < nn2; i++){
                    if(i>0){
                        if((argvs.bulkShipping_flag == false && (customers.at(veh2.route.at(i-1)).custno != customers.at(veh2.route.at(i)).custno)) ||
                        (argvs.bulkShipping_flag == true && (customers.at(customers.at(veh2.route.at(i-1)).tieup_customer.at(0)).custno != customers.at(customers.at(veh2.route.at(i)).tieup_customer.at(0)).custno))){
				...
				for(int i=start_idx2+1; i<nn2; i++){
                    if((argvs.bulkShipping_flag == false && (customers.at(veh2.route.at(i-1)).custno != customers.at(veh2.route.at(i)).custno)) ||
                        (argvs.bulkShipping_flag == true && (customers.at(customers.at(veh2.route.at(i-1)).tieup_customer.at(0)).custno != customers.at(customers.at(veh2.route.at(i)).tieup_customer.at(0)).custno))){
                            atpen += max(0.0, veh2.arr.at(i) - customers.at(veh2.route.at(i)).due_fs);
                    }
                }
                atpen += max(0.0, lv2-veh2.endhour);
            ...
            if(argvs.avevisits_ape_flag == true || argvs.avevisits_ae_flag == true){
                ...
                if(argvs.avevisits_ape_flag == true){
                    ...
                    avisits_error = calc_avevisits_AE(vehicles, customers, mvehicles, argvs.avevisits_ae, argvs.bulkShipping_flag);
                }
            }
            ...
            if(argvs.avedist_ape_flag == true || argvs.avedist_ae_flag == true){
                ...
                for(int i = 0; i < vnum; i++){
                    if(vehicles.at(i).vehno == vehicle1.vehno){
                        mvehicles.emplace(i, vehicle1.totalcost+adist1-bdist1);
                    }
                    if(vehicles.at(i).vehno == vehicle2.vehno){
                        mvehicles.emplace(i, vehicle2.totalcost+adist2-bdist2);
                    }
                }
            ...
            if(argvs.opt_unassigned_flag==true){
                if(vehicle1.reserved_v_flag==true){
                    int route_size = skv1_route.size();
                    for(int rc=0; rc<route_size; rc++){
                ...
                if(vehicle2.reserved_v_flag==true){
                    int route_size = skv2_route.size();
                    for(int rc=0; rc<route_size; rc++){
